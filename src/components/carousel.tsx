    ...
    return (
      <div className='w-full max-w-full p-0 overflow-hidden sm:px-12 md:max-w-6xl md:mx-auto'>
            <div className='py-6 px-4 sm:py-8 md:py-14'>
                <h1 className='top-0 text-3xl sm:text-4xl md:text-5xl font-extrabold text-[#0F0F11]'>Welcome to Nice Gadgets store!</h1>
            </div>
        <Carousel
        setApi={setApi}
        ...
        >
            ...
            <CarouselPrevious 
              className="hidden sm:flex sm:left-4 sm:bg-white/30 sm:hover:bg-white/50 sm:border-0 sm:text-white sm:backdrop-blur-sm" 
            />
            <CarouselNext 
              className="hidden sm:flex sm:right-4 sm:bg-white/30 sm:hover:bg-white/50 sm:border-0 sm:text-white sm:backdrop-blur-sm" 
            />
        </Carousel>
        ...
        {Array.from({ length: count }).map((_, index) => (
          <button
            ...
            className={`transition-all duration-300 rounded-full cursor-pointer ${
              index === current 
                ? "bg-[#0F0F11] h-2 w-6"  // Ativo: mais largo e escuro
                : "bg-gray-300 h-2 w-2 hover:bg-gray-400" // Inativo: bolinha cinza
            }`}
